...
var positiveFeedbackInput = $("#ruleFeedback").find(
		"input[name='positiveFeedback']");
var negativeFeedbackInput = $("#ruleFeedback").find(
		"input[name='negativeFeedback']");
var normalIndex = 4;//用来记录普通规则累积到的位置，在删除某个普通规则的时候重排的时候要修改此变量
var ruleHandler = {
	getTypeNo : {
		prop : function() {
			return 1;
	...
	altByTypeNo : function(typeNo){
		var s = "未定义";
		switch(typeNo){
			case 1:s='prop';break;
			case 2:s='gps';break;
			case 4:s='scan';break;
			case 0xfffff8:s='finality';break;
	...
	descByTypeNo : function(typeNo){
		var s = "未定义";
		switch(typeNo){
			case 1:s=this.desc['prop'];break;
			case 2:s=this.desc['gps'];break;
			case 4:s=this.desc['scan'];break;
			case 0xfffff8:s=this.desc['finality'];break;
			default:s=this.desc['normal'];break;
		}
		return s;
	}
}
$("#btnAddRule")
		.click(
				function() {
					...
					ruleList
							...
							.each(
									function() {
										...
										if (data) {
											if (data.type < 8
													&& data.type == rule.type) {// 前置，对比类型
												alertMsg.warn("不能重复添加前置规则！");
												isRepeat = true;
												return;
											} else if (data.type >= 8
													&& data.type != 2147483640) {// 关键字，对比关键字
												haveNormalRule = true;
					...
					console.log("haveNormalRule=" + haveNormalRule);
					...
					var liNode = $(str.sprintf(type,ruleHandler.desc[type] + meta,
							rule.positiveFeedback, rule.negativeFeedback));
...
function taskFormValidateCallback(form, callback, confirmMsg) {
	...
	var chainId = parseInt($form.find("input[name='taskChain.id']").val());
	...
	$("input:checkbox[name=supersId]:checked").each(function() {
		task.supers.push({
...
$.each(rules,function(){
	var str = "<li type='{0}'><p>{1}</p><p>正反馈:{2}</p><p>负反馈:{3}</p><button onclick='return false'>删除</button></li>";
	var liNode = $(str.sprintf(ruleHandler.altByTypeNo(this.type),ruleHandler.descByTypeNo(this.type) + this.meta,
			this.positiveFeedback, this.negativeFeedback));
	liNode.data("rule", this);// 绑定数据
	liNode.find("button").click(function() {
		liNode.remove();
		//重排普通规则
		resetNormalRule();
