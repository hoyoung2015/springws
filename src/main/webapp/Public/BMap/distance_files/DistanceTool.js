...
//            this._followTitle.setStyles({color : "#333", borderColor : "#ff0103"});
	...
	DistanceTool.prototype.removeAll = function(){
		console.log("removeAll");
		this._map.clearOverlays();  
		var event = new baidu.lang.Event("clearOverlays");
...
//                siblingPath.setPositionAt(1, poi);
            ...
            // 生成节点旁边的距离显示框
            var disText = disText = me._points[me._points.length - 1].lng+","+me._points[me._points.length - 1].lat;              
...
//                        lstP.setPositionAt(1, poiN);
    ...
    //huyang增加的函数，获取几个点的中心坐标
    DistanceTool.prototype._getCenterPoint = function(){
        var maxlng = 0;
        var minlng = 1000;
        var maxlat = 0;
        var minlat = 1000;
        for(i=0;i<this._points.length;i++){
        	if(maxlng < this._points[i].lng){
        		maxlng = this._points[i].lng
        	}
        	if(maxlat < this._points[i].lat){
        		maxlat = this._points[i].lat
        	}
        	if(minlng > this._points[i].lng){
        		minlng = this._points[i].lng
        	}
        	if(minlat > this._points[i].lat){
        		minlat = this._points[i].lat
        	}
        }
        return new BMap.Point((maxlng+minlng)/2,(maxlat+minlat)/2);
    };
...
//        label.setContent("<span style='" + this._styles.BMapLib_diso + "'><span style='" + this._styles.BMapLib_disi + "'>" + text + "</span></span>");
    ...
    DistanceTool.prototype._processLastOp = function() {
        ...
        /************************************/
      /*  var hpt0 =  new BMap.Point(me._points[0].lng, me._points[0].lat);
        var hpt1 =  new BMap.Point(me._points[me._points.length-1].lng, me._points[me._points.length-1].lat);
        var path = new BMap.Polyline([hpt0,hpt1], {enableMassClear : me._enableMassClear});
            me._map.addOverlay(path);
            me._paths.push(path);
            me._overlays.push(path);
            
            me._setSegDistance(hpt0,hpt1);//增加距离
            //设置线
            path.setStrokeWeight(me._opts.lineStroke);
            path.setStrokeColor(me._opts.lineColor);
            path.setStrokeOpacity(me._opts.opacity);
            path.setStrokeStyle(me._opts.lineStyle); */
        /************************************/
        // 保存本次测距对象
        var disObj = {};
        ...
        /**********/
       //me._paths[me._paths.length-1].setPositionAt(1, new BMap.Point(me._points[0].lng, me._points[0].lat));
        /**********/
...
//        disObj.closeBtn.setTitle("清除本次区域绘制");
        // 点击关闭按钮，绑定关闭按钮事件
